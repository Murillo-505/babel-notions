...
function EntityActionsMenu({
  ...
  return (
    <div ref={menuRef} className="entity-actions-menu relative shrink-0">
      ...
      {menuOpen ? (
        <div
          className="entity-actions-dropdown absolute right-0 top-full z-20 mt-1 min-w-[10rem] overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-xl"
          role="menu"
        >
          {isDeleting ? (
            <>
              <p className="px-3 py-2 text-xs text-zinc-400">{deletePrompt}</p>
              <button
                type="button"
                role="menuitem"
